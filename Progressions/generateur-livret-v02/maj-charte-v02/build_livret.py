...
BOX_LABEL = {"def":"Définition","regle":"Règle","prop":"Propriété","methode":"Méthode",
             ...
             "ressources":"Pour aller plus loin","rem":"Remarque"}
...
def render_comp(comp):
    ...
    for c in (x.strip() for x in comp.split(",")):
        if not c: continue
        lab = COMP_MAP.get(c, "")
        pills.append(f'<span class="chip"><b>{html.escape(c)}</b> {html.escape(lab)}</span>')
    return f'<div class="comps">{"".join(pills)}</div>'

ROMAN = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV"]
DOMAIN_NAME = {"N":"Nombres & calculs","C":"Calcul mental","G":"Géométrie & espace",
               "M":"Grandeurs & mesures","D":"Données & proportionnalité","A":"Algorithmique & programmation"}
DOMAIN_TH = {"N":("#9cc7e6","#3f78a3"),"G":("#ef9a8d","#bf4b39"),"D":("#8fd3b3","#3f8d6a"),
             "M":("#f0cf8e","#bd8b27"),"A":("#c9a9e0","#7d54a3"),"C":("#f3b59a","#c2693f")}
PENCIL = ('<svg class="pen" viewBox="0 0 24 24"><path d="M4 20l3.6-.8L19 8.8a1.8 1.8 0 0 0 0-2.6l-1.2-1.2'
          'a1.8 1.8 0 0 0-2.6 0L3.8 15.6 3 19.2z" fill="#fdebd2" stroke="#e0a23b" stroke-width="1.2"/>'
          '<path d="M14.2 6l3.8 3.8" stroke="#e0a23b" stroke-width="1.2"/></svg>')

def chap_header(code, title, niveau):
    dom = code[:1].upper()
    return (f'<div class="chap"><div class="deco-sakura"></div><div class="code">{html.escape(code)}</div>'
            f'<div class="ttl"><div class="eyebrow">{html.escape(DOMAIN_NAME.get(dom,""))}</div>'
            f'<h1>{html.escape(title)}</h1></div>'
            f'<div class="lvl">{html.escape(niveau)}</div></div>')

def _calc_lines(c, mode):
    out = []
    for j, ln in enumerate(c):
        t = ln.strip()
        m = re.match(r'^([A-Za-z]\w*)\s*=(.*)$', t)
        if m:
            idu, rest = m.group(1), m.group(2).strip()
            rhs = process(rest, mode)
            if j == len(c) - 1 and '[[' not in rest:
                rhs = f'<span class="res">{rhs}</span>'
            out.append(f'<span class="id">{html.escape(idu)}</span> = {rhs}')
        else:
...
def render_body(body, mode):
    ...
    for kind, data in body:
        if kind == "section":
            ...
            if btype == "raw":
                parts.append("\n".join(bbody)); continue
            if btype in ("exemple","exemples","ex"):
                parts.append(render_example(btitle, bbody, mode)); continue
            if btype == "reussite":
                items = "".join(f'<div class="rchk"><span class="rbox"></span>{process(l.strip(), mode)}</div>'
                                for l in bbody if l.strip())
                parts.append(f'<div class="box reussite"><span class="lab">{html.escape(btitle or "Critères de réussite")}</span>{items}</div>')
                continue
            if btype == "ressources":
                items = []
                for l in bbody:
                    ls = l.strip()
                    if ls.startswith("- "):
                        lab, _, url = ls[2:].partition("|"); lab, url = lab.strip(), url.strip()
                ...
                parts.append(f'<div class="box plus"><span class="lab">Pour aller plus loin</span>'
                             f'<ul class="ressources">{"".join(items)}</ul></div>')
                continue
            cls, deflabel = BOXMAP.get(btype, ("rem", btype.capitalize()))
            label = btitle or deflabel
            parts.append(f'<div class="box {cls}"><span class="lab">{inline(label)}</span>{lines_html(bbody, mode)}</div>')
    return "\n".join(parts)
...
FACE = {
 "ok":'<svg viewBox="0 0 24 24" class="face"><circle cx="12" cy="12" r="10" fill="#eef9f3" stroke="#3f8d6a" stroke-width="1.4"/><circle cx="8.5" cy="10.4" r="1.1" fill="#3f8d6a"/><circle cx="15.5" cy="10.4" r="1.1" fill="#3f8d6a"/><path d="M7.8 14q4.2 3.2 8.4 0" fill="none" stroke="#3f8d6a" stroke-width="1.5" stroke-linecap="round"/></svg>',
 "mid":'<svg viewBox="0 0 24 24" class="face"><circle cx="12" cy="12" r="10" fill="#fdf6e6" stroke="#bd8b27" stroke-width="1.4"/><circle cx="8.5" cy="10.4" r="1.1" fill="#bd8b27"/><circle cx="15.5" cy="10.4" r="1.1" fill="#bd8b27"/><path d="M8 15h8" fill="none" stroke="#bd8b27" stroke-width="1.5" stroke-linecap="round"/></svg>',
 "no":'<svg viewBox="0 0 24 24" class="face"><circle cx="12" cy="12" r="10" fill="#fdeef0" stroke="#c0537a" stroke-width="1.4"/><circle cx="8.5" cy="10.4" r="1.1" fill="#c0537a"/><circle cx="15.5" cy="10.4" r="1.1" fill="#c0537a"/><path d="M7.8 15.5q4.2 -3.2 8.4 0" fill="none" stroke="#c0537a" stroke-width="1.5" stroke-linecap="round"/></svg>',
}
...
def render_capable(seq, mode):
    ...
    head = (f'<tr><th class="cap-obj">Objectif</th><th>{FACE["ok"]}<br><span class="cap-h">oui&nbsp;!</span></th>'
    ...
    for o in objs:
        ...
        rows.append(f'<tr><td class="cap-obj">{_niv_badge(o.get("niveau","")) if mode!="eleve" else ""}{inline(o.get("texte",""))}</td>'
    ...
    return ('<div class="capable"><div class="cap-title">Je suis maintenant capable de&nbsp;…</div>'
            f'<table class="capgrid">{head}{"".join(rows)}</table>'
            '<div class="cap-note">Je mets une croix dans la case qui me correspond pour chaque objectif.</div></div>')
...
# ===================== POLICES + CSS CHARTE =====================
def font_faces_file():
    def ff(fam, wght, style, fn):
        p = FONT_DIR / fn
        if not p.exists(): return ""
        st = f"font-style:{style};" if style else ""
        return f"@font-face{{font-family:'{fam}';font-weight:{wght};{st}src:url('{p.as_uri()}');}}\n"
    return ("".join([
        ff("Round","300 700","","Baloo2.ttf"),
        ff("TitleC","200 700","","Oswald-VF.ttf"),
        ff("Body","300 800","","OpenSans-VF.ttf"),
        ff("Body","300 800","italic","OpenSans-Italic-VF.ttf"),
        ff("MathVar","400","italic","KaTeX_Math-Italic.ttf"),
        ff("Atkinson","400","","Atkinson-Regular.ttf"), ff("Atkinson","700","","Atkinson-Bold.ttf"),
        ff("Atkinson","400","italic","Atkinson-Italic.ttf"), ff("Atkinson","700","italic","Atkinson-BoldItalic.ttf"),
        ff("OpenDyslexic","400","","OpenDyslexic-Regular.otf"), ff("OpenDyslexic","700","","OpenDyslexic-Bold.otf"),
        ff("OpenDyslexic","400","italic","OpenDyslexic-Italic.otf"),
    ]))

def font_faces_b64(police="atkinson"):
    import base64 as _b64
    def ff(fam, wght, style, fn):
        p = FONT_DIR / fn
        if not p.exists(): return ""
        mime = "font/otf" if fn.lower().endswith(".otf") else "font/ttf"
        data = _b64.b64encode(p.read_bytes()).decode()
        st = f"font-style:{style};" if style else ""
        return f"@font-face{{font-family:'{fam}';font-weight:{wght};{st}src:url('data:{mime};base64,{data}');}}\n"
    out = [ff("Round","300 700","","Baloo2-s.ttf"),
           ff("TitleC","200 700","","Oswald-s.ttf"),
           ff("MathVar","400","italic","KaTeX_Math-Italic.ttf")]
    if police == "atkinson":
        out += [ff("Atkinson","400","","Atkinson-Regular.ttf"), ff("Atkinson","700","","Atkinson-Bold.ttf"),
                ff("Atkinson","400","italic","Atkinson-Italic.ttf"), ff("Atkinson","700","italic","Atkinson-BoldItalic.ttf")]
    elif police == "opendyslexic":
        out += [ff("OpenDyslexic","400","","OpenDyslexic-Regular.otf"), ff("OpenDyslexic","700","","OpenDyslexic-Bold.otf"),
                ff("OpenDyslexic","400","italic","OpenDyslexic-Italic.otf")]
    else:
        out += [ff("Body","300 550","","OpenSans-s.ttf"), ff("Body","551 800","","OpenSans-b.ttf"),
                ff("Body","300 800","italic","OpenSans-it.ttf")]
    return "".join(out)

BODYFAM = {"atkinson":"Atkinson", "opensans":"Body", "opendyslexic":"OpenDyslexic"}

FOOT = ""

CHARTER_CSS = r"""
:root{ --ink:#4a4458; --ink-soft:#6f6780; --line:#ece2f0;
  --body:@@BODY@@,'DejaVu Sans',sans-serif; --title:'TitleC','DejaVu Sans',sans-serif; --round:'Round','DejaVu Sans',sans-serif;
  --th:#9cc7e6; --thd:#3f78a3; }
*{ box-sizing:border-box; }
html,body{ margin:0; padding:0; }
body{ font-family:var(--body); color:var(--ink); font-size:@@SIZE@@pt; line-height:@@LH@@; }
.chapitre.chap-break{ break-before:page; page-break-before:always; }
.box,.crit,.ex,.calc,.capable,.objectifs{ break-inside:avoid; page-break-inside:avoid; }
.chap,.sec,.sub{ break-after:avoid; }
em{ font-style:italic; } sup{ font-size:.72em; } sub{ font-size:.72em; }
b,strong{ font-weight:700; color:var(--thd); }
p{ margin:.34em 0; } ul{ margin:.3em 0 .3em 1.1em; padding:0 0 0 .4em; } li{ margin:.24em 0; }

/* en-tete chapitre */
.chap{ position:relative; display:flex; align-items:center; gap:5mm; border-radius:22px; padding:4.2mm 6.5mm;
  margin:0 0 5mm; background:linear-gradient(120deg, color-mix(in srgb,var(--th) 34%,white), color-mix(in srgb,var(--th) 14%,white) 55%, #f3eefb);
  box-shadow:0 6px 18px #c9a9e01f; overflow:hidden; }
.chap .code{ flex:0 0 auto; font-family:var(--round); font-weight:700; color:#fff; background:var(--th);
  width:17mm; height:17mm; border-radius:16px; display:flex; align-items:center; justify-content:center;
  font-size:19pt; border:2.5px solid #ffffffcc; box-shadow:0 4px 10px #00000018; }
.chap .ttl{ flex:1; }
.chap .eyebrow{ font-family:var(--title); font-weight:600; text-transform:uppercase; letter-spacing:1.8px; color:var(--thd); font-size:9.5pt; }
.chap h1{ font-family:var(--round); font-weight:700; color:#5b4a63; font-size:20pt; line-height:1.05; margin:.6mm 0 0; }
.chap .lvl{ flex:0 0 auto; font-family:var(--round); font-weight:700; color:#fff; background:var(--th);
  border-radius:12px; padding:2mm 4.5mm; font-size:14pt; box-shadow:0 3px 8px #00000018; }
...
.chip b{ color:var(--thd); }

/* sections */
.sec{ display:flex; align-items:center; gap:9px; margin:4.5mm 0 2mm; }
...
.sec h2{ font-family:var(--round); font-weight:700; color:var(--thd); font-size:15pt; margin:0; }
...
.box p{ margin:1mm 0; } .box ul{ margin:1mm 0 0; padding-left:5mm; } .box li{ margin:.6mm 0; }
.box.def{ --bc:#f1d9e7; --bl:#dd87b0; --bg:#fdf4f8; }
.box.prop{ --bc:#cde9dd; --bl:#5fb98c; --bg:#f3fbf6; }
.box.meth{ --bc:#d3e6f3; --bl:#7bb0dd; --bg:#f1f7fc; }
.box.rem{ --bc:#e3def2; --bl:#a899dd; --bg:#f6f4fc; }
.box.rappel{ --bc:#e3def2; --bl:#a899dd; --bg:#f7f5fc; border-left-style:dashed; }
.box.retenu{ --bc:#f3d79a; --bl:#e6b450; --bg:#fff8ea; }
.box.plus{ --bc:#f3d79a; --bl:#e6b450; --bg:#fff8ea; }
...
.box.reussite .rchk{ display:flex; gap:8px; align-items:flex-start; margin:1.2mm 0; }
.box.reussite .rbox{ width:13px; height:13px; border:2px solid #df8a4e; border-radius:4px; flex:0 0 auto; margin-top:1.4px; background:#fff; }
.box.objectifs .lab{ background:var(--th); }
.obj-intro{ font-style:italic; color:var(--ink-soft); margin:.1em 0 .3em; }
.obj-list{ list-style:none; margin-left:.1em; padding-left:0; } .obj-list li{ margin:.32em 0; }
.niv-badge{ display:inline-block; color:#fff; font-size:.64em; font-weight:700; border-radius:5px; padding:1px 6px; margin-right:6px; vertical-align:.12em; font-family:var(--round); }
...
.id{ font-family:var(--mathv,'MathVar'),serif; font-style:italic; color:var(--thd); font-weight:700; }
.math em,.mit{ font-family:'MathVar',serif; font-style:normal; }
...
.crit h3{ font-family:var(--round); font-weight:700; color:#c2693f; font-size:12pt; margin:0 0 1.8mm; }
.chk{ display:flex; align-items:flex-start; gap:8px; margin:1.3mm 0; }
.chk .pen{ width:15px; height:15px; flex:0 0 auto; margin-top:1px; }

/* je suis capable de */
.capable{ margin:3.5mm 0 1mm; border:2px solid var(--th); border-radius:14px; padding:3mm 4mm;
  background:color-mix(in srgb,var(--th) 6%,white); }
.cap-title{ font-family:var(--round); font-weight:700; color:var(--thd); font-size:12.5pt; margin-bottom:2mm; }
.capgrid{ border-collapse:collapse; width:100%; }
.capgrid th,.capgrid td{ border:1.2px solid #cdbad8; padding:4px 7px; text-align:center; }
.capgrid th{ background:color-mix(in srgb,var(--th) 16%,white); font-family:var(--round); font-weight:700; color:var(--thd); font-size:8.6pt; vertical-align:middle; }
.cap-obj{ text-align:left !important; } .cap-h{ display:inline-block; }
.face{ width:1.7em; height:1.7em; vertical-align:middle; }
.cap-tick{ width:13mm; height:8mm; background:#fffdfe; } .cap-exo{ width:3.4em; }
.cap-note{ font-size:.78em; color:var(--ink-soft); font-style:italic; margin-top:2mm; }

/* fractions, racine */
.frac{ display:inline-block; vertical-align:-.45em; text-align:center; margin:0 .14em; }
.frac .fnum,.fnum{ display:block; padding:0 .3em; border-bottom:1.6px solid currentColor; line-height:1.1; }
.frac .fden,.fden{ display:block; padding:0 .3em; line-height:1.1; }
.sqrtarg{ border-top:1.5px solid currentColor; padding:0 .15em; }

/* ressources + qr */
.ressources{ list-style:none; margin:0; padding:0; }
.ressources li{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:4px 0; border-bottom:1px dotted #e3d8ef; }
.res-url{ font-size:.78em; color:#9a8ea8; }
.qr{ height:3em; } .qr-missing{ font-size:.8em; color:#aaa; }
...
def build_print_css(police, taille, interligne, couleur, foot=""):
    ...
    page = ("@page{ size:A4; margin:17mm 15mm 16mm;"
            "@bottom-left{ content:'" + (foot or "").replace("'", "\u2019") + "';"
            "font-family:'TitleC','DejaVu Sans',sans-serif; font-weight:500; font-size:8.2pt; color:#b29ec2; }"
            "@bottom-right{ content:counter(page) ' / ' counter(pages);"
            "font-family:'Round','DejaVu Sans',sans-serif; font-weight:600; font-size:8pt; color:#fff;"
            "background:#cdb4e0; border-radius:999px; padding:1.5px 10px; } }")
    bodyfam = BODYFAM.get(police, "Atkinson")
    css = CHARTER_CSS.replace("@@SIZE@@", f"{taille}").replace("@@LH@@", f"{interligne}").replace("@@REP@@", "#c0698e" if not nb else "#222").replace("@@BODY@@", f"'{bodyfam}'")
    return font_faces_b64(police) + "\n" + page + "\n" + css + "\n" + cover_css(nb) + cover_vacances_css()

def build_web_css(police):
    bodyfam = BODYFAM.get(police, "Atkinson")
    css = CHARTER_CSS.replace("@@SIZE@@", "11.5").replace("@@LH@@", "1.5").replace("@@REP@@", "#c0698e").replace("@@BODY@@", f"'{bodyfam}'")
...
.idbox {{ background:{'#f4f4f4' if nb else '#f6f0fb'}; border-radius:10px; padding:8px 14px; font-size:.95em; }}
.cover-title {{ font-family:'Round',cursive; font-size:46pt; color:{'#222' if nb else '#5b4a63'}; margin:4mm 0 0; font-weight:700; }}
.cover-sub {{ font-family:'Round',cursive; font-size:28pt; color:{'#444' if nb else '#c0698e'}; margin-bottom:2mm; }}
.cover-animal {{ width:132px; height:auto; margin:2mm auto; display:block; }}
.cover-periode {{ width:78%; margin:4mm auto 0; background:{'#f6f6f6' if nb else '#f6f0fb'};
  border:2px dashed {accent}; border-radius:14px; padding:10px 16px; text-align:left; }}
.periode-label {{ font-family:'Round',cursive; font-size:24pt; text-align:center; color:{'#222' if nb else '#5b4a63'}; margin-bottom:4px; }}
.sommaire {{ list-style:none; margin:0; padding:0; }}
.sommaire li {{ display:flex; align-items:center; gap:9px; margin:5px 0; }}
.case-somm {{ width:1em; height:1em; border:1.6px solid {'#555' if nb else '#c9a9e0'}; border-radius:3px; flex:0 0 auto; }}
.c-code {{ font-weight:bold; color:{'#222' if nb else '#7d54a3'}; min-width:2.4em; }}
.cover-foot {{ display:flex; justify-content:center; gap:18px; align-items:baseline; margin-top:7mm; }}
.cover-foot .prof {{ font-family:'Round',cursive; font-size:22pt; color:{'#222' if nb else '#c0698e'}; }}
.cover-foot .annee {{ letter-spacing:2px; color:#888; }}
"""
...
.cv-title { font-family:'Round',sans-serif; font-size:46pt; color:#6A4C9C; margin:2mm 0 0; font-weight:700; line-height:1.05; }
.cv-sub { font-family:'Round',sans-serif; font-size:21pt; color:#C77B9E; }
.cv-by { color:#8a7ea0; margin-top:2mm; font-size:.95em; }
.cv-animal { display:block; width:60%; max-width:300px; margin:7mm auto 4mm; }
.cv-appartient { width:74%; margin:0 auto 4mm; border:2px dashed #C9B6E0; border-radius:16px; padding:12px 18px; text-align:left; color:#5a5168; }
.cv-somm-box { width:80%; margin:1mm auto; background:#FFF7FB; border:2px dashed #E3B7CE; border-radius:18px; padding:9px 20px 12px; }
.cv-somm-title { font-family:'Round',sans-serif; color:#6A4C9C; font-size:18pt; text-align:center; margin-bottom:3px; }
