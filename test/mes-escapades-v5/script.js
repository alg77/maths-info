...
function fillCard(node,p){
  ...
  if(p.link){ const linkRow=document.createElement("div"); linkRow.className="card-link-row"; linkRow.innerHTML=`<a class="video-link" href="${p.link}" target="_blank" rel="noreferrer">🎥 Lien sauvegardé</a>`; node.querySelector(".tags").after(linkRow); }
...
function openAddModal(){ const modal=document.getElementById("add-modal"); if(modal){modal.classList.add("open");modal.setAttribute("aria-hidden","false"); const input=modal.querySelector("input[name='name']"); setTimeout(()=>input&&input.focus(),80);} }
function closeAddModal(){ const modal=document.getElementById("add-modal"); if(modal){modal.classList.remove("open");modal.setAttribute("aria-hidden","true");} }
function setup(){
  ...
  document.querySelectorAll(".open-add").forEach(b=>b.onclick=openAddModal);
  const modal=document.getElementById("add-modal"); if(modal){modal.addEventListener("click",e=>{if(e.target===modal) closeAddModal();}); const close=modal.querySelector(".modal-close"); if(close) close.onclick=closeAddModal;}
  document.addEventListener("keydown",e=>{if(e.key==="Escape") closeAddModal();});
  const topBtn=document.querySelector(".back-to-top"); if(topBtn){topBtn.onclick=()=>window.scrollTo({top:0,behavior:"smooth"}); window.addEventListener("scroll",()=>topBtn.classList.toggle("visible",window.scrollY>520));}
  ...
  function addPlaceFromForm(form){ const f=new FormData(form); const id="custom-"+Date.now(); const rawTime=String(f.get("time")||"60 min"); const link=String(f.get("link")||"").trim(); const note=String(f.get("note")||"").trim(); const place={id,name:f.get("name"),city:f.get("city")||"À compléter",timeMin:parseInt((rawTime.match(/\d+/)||[60])[0],10),distance:f.get("distance")||"À compléter",category:f.get("category")||"resto",emoji:link?"🎥":"📍",cost:"€€",dog:"À compléter",desc:note || "Idée ajoutée rapidement. À compléter après la prochaine recherche ou visite.",tags:["ajout perso","wishlist"],link:link,scores:{overall:7.5,kid4:3,preteen:3,photo:3,japan:0,dog:2,budget:3,fatigue:2,food:2}}; const custom=loadCustom(); custom.push(place); saveCustom(custom); updateUser(id,{wishlist:true,comment:note}); form.reset(); if(form.id==="quick-add-form"){ closeAddModal(); } render(); }
  document.getElementById("add-form").onsubmit=e=>{e.preventDefault(); addPlaceFromForm(e.target);};
  const quickForm=document.getElementById("quick-add-form"); if(quickForm){quickForm.onsubmit=e=>{e.preventDefault(); addPlaceFromForm(e.target);};}
