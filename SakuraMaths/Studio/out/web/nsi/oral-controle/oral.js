(function(){
  ...
  function initEvaluation(selects){
    ...
    document.querySelector('#open-evaluation').addEventListener('click',()=>{
      const exercises=selects.map(s=>byId.get(s.value)),warning=document.querySelector('.warning'),candidate=document.querySelector('#candidate-name').value.trim();
      if(!candidate){warning.textContent='Indique le nom du candidat.';return;}
      if(!exercises[0]||!exercises[1]||exercises[0].theme===exercises[1].theme){warning.textContent='Choisis deux exercices appartenant à deux thèmes différents.';return;}
      warning.textContent='';document.querySelector('#evaluation-title').textContent=candidate;
    ...
    document.querySelector('#save-evaluation').addEventListener('click',()=>{
      ...
      const record={id:crypto.randomUUID?.()||String(Date.now()),candidate,date:new Date().toISOString(),exercices:[panel.dataset.exercise1,panel.dataset.exercise2],scores,note:updateMark(),notes:document.querySelector('#evaluation-notes').value};
      const key='sakura-oral-controle-evaluations',records=JSON.parse(localStorage.getItem(key)||'[]');records.push(record);localStorage.setItem(key,JSON.stringify(records));
      ...
      document.querySelector('#save-status').textContent=`Évaluation enregistrée — ${record.note.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})}/20`;
    });
    document.querySelector('#reset-evaluation').addEventListener('click',()=>{
      panel.hidden=true;document.querySelector('#candidate-name').value='';selects.forEach(s=>s.value='');grid.querySelectorAll('input[value="0"]').forEach(input=>input.checked=true);document.querySelector('#evaluation-notes').value='';document.querySelector('#save-status').textContent='';window.scrollTo({top:document.querySelector('.composer').offsetTop-20,behavior:'smooth'});
    });
  }
